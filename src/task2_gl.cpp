...
const char* fragmentShaderSource = "\n" 
	...
	"    FragColor = mix(texture(ourTexture1, UpDownCoord), texture(ourTexture2, UpDownCoord), 0.2);\n" 
	"}";
...
int main()
{
	...
	unsigned char *data2 = stbi_load("test/test2.jpg", &width2, &height2, &nrChannels2, 0);
	printf("width is2 %d, height2 is %d,nrChannels2 is %d\r\n",width2,height2,nrChannels2);
	...
	object1.setInt(1,"ourTexture2", 1);
	...
	{  
		...
		// glBindTexture(GL_TEXTURE_2D, texture);				//全程没有第二个对象，可以不需要任何解绑
		// glBindTexture(GL_TEXTURE_2D, texture2);
		// glBindVertexArray(VAO);
